...
const EASY_EMOTIONS: EmotionItem[] = [
  { id: 'happy', name: 'Happy', emoji: '😊', description: 'Feeling good and joyful', difficulty: 'easy' },
  { id: 'sad', name: 'Sad', emoji: '😢', description: 'Feeling down or upset', difficulty: 'easy' },
  { id: 'angry', name: 'Angry', emoji: '😠', description: 'Feeling mad or frustrated', difficulty: 'easy' },
  { id: 'scared', name: 'Scared', emoji: '😨', description: 'Feeling afraid', difficulty: 'easy' },
  { id: 'surprised', name: 'Surprised', emoji: '😲', description: 'Something unexpected happened', difficulty: 'easy' },
  { id: 'tired', name: 'Tired', emoji: '😴', description: 'Feeling sleepy', difficulty: 'easy' },
  { id: 'love', name: 'Love', emoji: '🥰', description: 'Feeling love and care', difficulty: 'easy' },
];

const HARD_EMOTIONS: EmotionItem[] = [
  { id: 'confused', name: 'Confused', emoji: '😕', description: 'Not understanding something', difficulty: 'hard' },
  { id: 'excited', name: 'Excited', emoji: '🤩', description: 'Very happy about something', difficulty: 'hard' },
  { id: 'silly', name: 'Silly', emoji: '🤪', description: 'Being playful and funny', difficulty: 'hard' },
];
...
export default function EmotionsGame() {
  ...
  const [target, setTarget] = useState<EmotionItem>(EASY_EMOTIONS[0]);
  ...
  const generateRound = useCallback(() => {
    ...
    const pool = difficulty === 'hard' ? HARD_EMOTIONS : EASY_EMOTIONS;
    const allPool = [...EASY_EMOTIONS, ...HARD_EMOTIONS];
    
    const available = pool.filter(e => !usedEmotions.includes(e.id));
    const itemPool = available.length >= 1 ? available : pool;
    
    const shuffled = [...itemPool].sort(() => Math.random() - 0.5);
    const newTarget = shuffled[0];
    const wrongPool = allPool.filter(e => e.id !== newTarget.id);
    ...
    setUsedEmotions(prev => [...prev, newTarget.id]);
  }, [usedEmotions, adaptiveDifficulty]);
  ...
  useEffect(() => {
    if (gameComplete && childId && user) {
      saveGameSession();
      updateStats(correctAnswers, TOTAL_ROUNDS, 'emotions');
      updateProgress(childId);
      if (childName) {
        const accuracy = Math.round((correctAnswers / TOTAL_ROUNDS) * 100);
        checkForMilestones(childId, childName, 'Emotions', score, score, accuracy, maxStreak);
      }
    }
  }, [gameComplete]);
  ...
  const saveGameSession = async () => {
    ...
    try {
      await supabase.from('game_sessions').insert({
        child_id: childId, game_type: 'emotions', score,
  ...
  const handleSelect = (item: EmotionItem) => {
    ...
    if (item.id === target.id) {
      ...
      setMistakes(prev => [...prev, { question: `Find ${target.name}`, correctAnswer: target.name, userAnswer: item.name, category: 'Emotions' }]);
      aiTrackWrong({ gameType: 'emotions', score, correctAnswers, wrongAnswers: wrongAnswers + 1, totalQuestions: round });
  ...
  const resetGame = () => {
    ...
    setGameComplete(false); setUsedEmotions([]); adaptiveDifficulty.reset();
  ...
  if (gameComplete) {
    return (
      <GameFinishScreen childId={childId || ''} score={score} accuracy={accuracy}
        ...
        gameTitle="Emotions Game" gameEmoji="😊" onPlayAgain={resetGame}
        gradientFrom="from-pastel-rose/30" gradientTo="to-pastel-lavender/30" />
    );
  }
    ...
    <div className="min-h-screen bg-gradient-to-b from-pastel-rose via-background to-pastel-lavender overflow-hidden page-enter">
      ...
      <main className="max-w-lg mx-auto px-4 pb-8">
        <div className="text-center mb-8">
          <p className="font-display text-xl text-foreground mb-2">Which face shows someone who is:</p>
          <div className="bg-card rounded-2xl p-6 shadow-card">
            <p className="font-display text-2xl font-bold text-duo-pink mb-1">{target.name}</p>
            ...
            <button key={option.id} onClick={() => handleSelect(option)}
              ...
              className={cn(
                ...
                selected === option.id
                  ...
                  : "border-border hover:border-duo-pink",
        ...
        <div className="flex justify-center">
          <GuideMascot
            message={showCelebration ? "You know your feelings! Amazing! 💝" : isCorrect === false ? "That's a different feeling. Try again!" : "Look at the faces carefully!"}
